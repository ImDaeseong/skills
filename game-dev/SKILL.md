...
- **AetherAI** — [aetherforgeai.com](https://www.aetherforgeai.com/en) (checked 2026-08-27, real product confirmed via WebSearch), a paid SaaS implementation of Step 3.2's anchor-pose pattern (single character image expanded into a multi-directional sprite sequence). Not a GitHub repo, so LAW 1's star-count ranking doesn't apply and no `LICENSE` exists to verify — `[LICENSE-UNCONFIRMED]`. Its specific animation-category claims (attack/pickup/fall/idle/walk/run) come from a single vendor promotional video with no independent or academic verification — `[LOW-EVIDENCE]`. Named here only as one known implementation of an already-evidenced technique, not as the source of the technique itself.
...
Run `vibe-coder`'s Step 3 plan gate (see that skill) with game-specific content as the subject matter — a one-page design doc covering the unit/entity tree (stats, roles), resource/economy structure, and enemy/AI behavior patterns, reviewed and signed off before any gameplay code is written. No dedicated GitHub game-design-document methodology cleared this workspace's evidence bar (searched 2026-07-19: the highest-starred candidates were a curated link list and single-digit/low-tens-star personal templates, and the AI-assisted `game-design-doc-generator` doesn't even cover unit/economy/behavior content) — reuse the already-evidenced `vibe-coder` plan-gate methodology (`obra/superpowers`, see `vibe-coder` Attribution) instead of citing a source this weak.
