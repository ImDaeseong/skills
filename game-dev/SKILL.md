...
1. **Which engine, or is this greenfield?** If the user already has a project, the engine is fixed — read its existing structure before proposing anything. If greenfield, the choice depends on target platform (2D web/mobile → Phaser/PixiJS; cross-platform indie 2D/3D → Godot; AAA-adjacent 3D or an existing UE project → Unreal; broad commercial 3D with a large asset-store ecosystem → Unity).
