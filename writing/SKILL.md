...
**Language check first:** if the text is Korean, use `epoko77-ai/im-not-ai`'s 70-pattern Korean taxonomy (see Attribution) instead of the English catalog below — the two pattern sets do not overlap. The `Identify → Draft → Self-check → Final` shape stays the same; only the pattern list changes.

Follow `blader/humanizer`'s own process (see Attribution) rather than reinventing it:

1. **Identify** — scan the text against the catalogued patterns (em dashes, inflated-significance language, rule-of-three, inline-header bullet lists, sycophantic tone, knowledge-cutoff disclaimers, filler phrases, and the rest — read the source for the complete list with before/after examples).
