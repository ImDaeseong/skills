...
- **gstack** — [github.com/garrytan/gstack](https://github.com/garrytan/gstack) (checked 2026-07-18 via direct GitHub HTML verification — GitHub API was rate-limited at check time, so the star counter was read directly from the repo page's `aria-label`, not estimated), MIT license (verified same way). Corrects an earlier "not verified in this pass" placeholder in this table — LAW 0 requires a real number, not a deferred check that never got done. Only the `/spec` and `/plan-ceo-review` *question frameworks* are borrowed in Step 3 — gstack's own runtime (`~/.gstack/`, `gstack-config`, telemetry binaries) is intentionally excluded; those binaries don't exist outside a gstack install and would error or silently no-op if invoked here.
- **compound-engineering-plugin** — [github.com/EveryInc/compound-engineering-plugin](https://github.com/EveryInc/compound-engineering-plugin) (checked 2026-07-18 via direct GitHub HTML verification), MIT license. Evaluated per LAW 1 as an alternative research-to-roadmap reference (its `ce-plan` skill turns research into a structured plan) — the same Matt Van Horn who built `last30days` is associated with this project too. Ranks well below gstack by star count, so gstack remains the primary Step 3 reference; noted here as the documented second candidate rather than a single unexplained pick, per LAW 1.
- **Firecrawl** — [github.com/firecrawl/firecrawl](https://github.com/firecrawl/firecrawl) (checked 2026-07-18 via GitHub API), AGPL-3.0 license (verified via GitHub API license endpoint). Optional bulk-scraping tool named in Step 1.3 — used as an external API/tool call when a key is configured, not vendored or self-hosted by this skill.
- **Negative test case for Step 1.4a**: useful hypotheses were mixed with self-reported revenue/research hours and universal percentages without disclosed samples. The tactics were not adopted as facts; the case was cross-checked against [Instagram's official recommendation guidance](https://www.facebook.com/help/instagram/653964212890722), [Klaviyo's abandoned-cart benchmarks](https://www.klaviyo.com/blog/abandoned-cart-benchmarks), and published research on [money-back guarantees and purchase intention](https://www.sciencedirect.com/science/article/pii/S0148296313003603).
- **Step 1.1a**: internal/uploaded business data (support tickets, spreadsheets, competitor research already collected) treated as a first-class evidence source alongside last30days, not a fallback reached for only when public research is thin. NotebookLM itself is proprietary and not adopted.
- **Step 1.5a**: the question of whether a service-shaped idea should be repackaged as a productized micro-SaaS (frontend UI + AI agent + automation, connected end to end) instead of sold as bespoke service work each time. Zapier and Lovable (the tools involved in the source example) are proprietary, not adopted.
- **Step 1.5b validation-sequencing framing**: validate with a cheap real-world probe (landing page + a few days of paid ad spend, or one manually-delivered sale) before committing to a full build, not after. A 30-day sequence (idea → minimal site → ad-test → first customer → then content/polish) is the evidence for the ordering, not a specific tool adopted here.
...
- **Step 1.5c pricing/business-model framing**: for an agent-SaaS idea specifically (the product is a job, not just software), start pricing by selling the pilot like labor — a setup fee plus a flat monthly fee to a first handful of customers in one niche — then move toward usage- or outcome-based pricing once the delivered value is actually measured, rather than starting there. Also source of the "own the workflow" framing: the durable moat is owning the end-to-end job (the wrapper — logs, approval gates, evals, analytics — around the model call), not the underlying model capability, which echoes the same defensibility check already in Step 1.5 (see Attribution, a16z) rather than introducing a separate one.
- **US Census Bureau, Nonemployer Statistics** — [census.gov/programs-surveys/nonemployer-statistics.html](https://www.census.gov/programs-surveys/nonemployer-statistics.html), government first-party data (2023 figures, published 2025; checked 2026-07-29), not a GitHub repo, no LAW 1 star/license ranking applies. Source of Step 1.5a-solo's scale claim: 30,427,808 US nonemployer establishments in 2023 (up from 29,811,495 in 2022), generating roughly \$1.8 trillion in revenue (~6.4% of US GDP) — cited to establish that single-operator businesses are a large, government-measured category, not an edge case this skill can treat as an afterthought.
- **Paul Jarvis, *Company of One*** (Mariner Books/Houghton Mifflin Harcourt, 2019; UK edition Penguin) — a widely-cited business book, not a GitHub repo, no LAW 1 ranking applies. Source of Step 1.5a-solo's "deliberately small vs. grow-or-die" framing: staying intentionally small can be the goal rather than a constraint to escape, which reweights which of Step 1.5's four criteria matter for a solo-operator idea.
- **Indie Hackers** — [indiehackers.com](https://www.indiehackers.com), community founded by Courtland Allen in 2016 (verified 2026-07-29), not a GitHub repo, no LAW 1 ranking applies. Named in Step 1.5a-solo as a real, citable source of solo/small-team products with public revenue disclosures — a comparable-idea research angle `last30days`'s generic sources don't specifically target.
- **Webster & Wind, "A General Model for Understanding Organizational Buying Behavior"** — *Journal of Marketing* 36:2 (April 1972), pp. 12-19; 915 citations per Semantic Scholar (checked 2026-07-29) — not a GitHub repo, no LAW 1 ranking applies, still the standard reference cited in current B2B-marketing literature. Source of Step 3a's buying-center check: an organizational (B2B) purchase is a multi-role group decision — Initiator, User, Influencer, Decider, Approver, Buyer, Gatekeeper — not a single-consumer decision, so "who is affected" for a B2B idea needs the user, economic buyer, and blocker named separately rather than collapsed into one persona.
...
**1.4 Evidence gate — do not proceed on empty research.** Before moving to Step 2, check: did 1.1 and/or 1.2 return at least a handful of real, citable items? If both came back empty or "nothing solid this window," STOP. Do not run the council on zero evidence — that just produces five confident-sounding guesses. Tell the user research came back thin and ask whether to: narrow/rephrase the topic, supply their own context/data manually, or proceed anyway with everything downstream tagged `[LOW-EVIDENCE]`. This gate, plus 1.5b's real-signal probe below, are this skill's own loop-engineering checkpoints for business selection/feasibility — a halt condition and a re-run trigger around repeated research passes, the same discipline `agent-builder`'s Repeated-loop control guard names explicitly for build loops (see that skill's Attribution for the term's origin — not re-cited here to avoid duplicating the same paragraph across skills).
...
**1.5a Delivery shape: productized micro-SaaS vs. bespoke service.** If the idea is service-shaped (consulting, done-for-you automation, freelance delivery), ask whether it should instead be packaged as a repeatable product — a thin frontend UI wrapped around the same AI-agent-plus-automation logic — rather than sold as one-off bespoke work each time. This is a delivery-shape question, separate from the four selection criteria above: a service can pass all four and still not scale, because each new customer requires redoing the work by hand. Not every idea should be productized (some genuinely need bespoke delivery), so raise it as a question, not a default.

**1.5a-solo Ground a solo-operator idea in real data, not vibes.** If the user is explicitly building alone (no co-founder, no hires planned), don't default to a startup-scale framing that assumes a team — the single-operator model is itself a large, well-documented category, not a fringe case: the US alone had **30.4 million nonemployer establishments in 2023** (Census Bureau Nonemployer Statistics, up from 29.8M in 2022), generating **~\$1.8 trillion in revenue (about 6.4% of US GDP)** — see Attribution. Two reference frames, cited rather than assumed, for scoping the idea against this evidence:
- **Deliberately small vs. grow-or-die** (see Attribution, Paul Jarvis's *Company of One*): ask whether the idea's actual goal is "the biggest it can sustainably be run by one person" rather than "as big as possible" — this changes which of Step 1.5's four criteria matter most (personal fit and defensibility outweigh raw growth-rate when there's no team to scale hiring into).
- **Indie Hackers precedent** (see Attribution) is a real, citable source of solo/small-team products with public revenue numbers — when Step 1.1's research needs comparables for a solo-operator idea specifically, check whether `last30days` surfaced anything from that community, or search it directly, rather than only pulling from generic startup press that assumes venture-scale teams.
This is a scoping lens, not a fourth selection criterion — it doesn't replace Step 1.5's growing/low-competition/personal-fit/defensibility scoring, it calibrates what "good" looks like on those four when the operator count is fixed at one.

**1.5b Validate with real signal before committing to a full build.** Once an idea and delivery shape are picked, don't let research-stage confidence substitute for a real-world test — a small, cheap probe (a landing page plus a few days of paid ad spend, or a manually-delivered version sold to one real customer) surfaces actual demand faster and cheaper than building the full product first (a real-world timeline sequences exactly this: idea → build a minimal site → market-test with paid ads → land one real customer → only then invest in polish/content). For a tactical claim from Step 1.4a, define an A/B test or sequential test with the target segment, baseline, changed variable, primary metric, minimum observation window/sample, and stop condition before recommending it. Route the actual build to `design-report`/`vibe-coder` and the ad-test/channel work to `distribution` — this step's job is just to flag that the test should happen before the full build, not after.

**1.5c Pricing an agent-SaaS idea specifically.** If the delivery shape (1.5a) is an AI agent doing a job end-to-end rather than a self-serve software tool, don't default to a flat per-seat SaaS price — price it like labor first (see Attribution): a setup fee plus a flat monthly fee, sold to a first handful of customers within one niche, learning the actual delivered value before generalizing pricing. Once that value is measurable (units of work completed, outcomes hit), consider moving toward usage- or outcome-based pricing rather than staying flat-fee indefinitely — but treat that as a later move earned by real usage data, not the opening price. Note explicitly to the user that the durable part of the business is owning the workflow end-to-end (including the trust-building wrapper around the model call — logs, approval gates, evals, analytics), not the raw model call itself, which is the same defensibility question already asked in Step 1.5's fourth criterion, not a new one.
...
1. Frame the business question using the Step 1 research (1.1 web/social evidence plus any 1.2 KR-web supplement) as grounding context (not the user's raw question alone — enrich it with the strongest 3-5 evidence items, cited). This framing step is where prompt engineering actually matters for this skill — a vague raw question sent to all 5 advisors produces 5 vague answers; the enriched, evidence-grounded prompt is what makes the peer-review round (item 3) catch real blind spots instead of restating generic advice.
...
1. Who is affected? (be specific — a demographic/behavior segment found in Step 1 research, not "everyone") — **if this idea serves an organization rather than an individual, name the user, economic buyer, and blocker separately; they are often not the same person** (see Attribution, Webster & Wind's buying-center model — Initiator/User/Influencer/Decider/Approver/Buyer/Gatekeeper). "IT managers at mid-size companies" can stop short of the actual budget-holder or the gatekeeper who can block the deal — a well-documented B2B failure mode this check exists to catch before it reaches Step 3c's pricing/positioning work.
...
5. How will you know it worked? (a measurable signal, not a vibe) — anchor this to the specific funnel stage the idea actually operates in (Attract → Engage → Convert → Monetize → Retain), not a generic metric. A lead-gen idea's signal is an Attract/Engage number; a pricing change's signal is Convert/Monetize; a churn fix's signal is Retain. Naming the stage forces a specific metric instead of a vague "more revenue."
