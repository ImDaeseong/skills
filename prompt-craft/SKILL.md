...
- **Jeremy Utley** (Stanford d.school, Adjunct Professor of AI and Design Thinking, 12 years as Director of Executive Education at the Hasso Plattner Institute of Design — verified via `jeremyutley.com` and Capgemini interview series, 2026-07-29) — not a GitHub repo, cited for two concrete under-specification checks not covered elsewhere in this skill: give the model at least as much context as a new hire would need (his benchmark: 400+ words is a floor, not a target, for a non-trivial task), and the "humanity test" — write the prompt down and hand it to a human colleague; if they couldn't do the task from that text alone, the model's failure isn't a model problem.
- **이제현 (Lee Je-hyun)** (Microsoft MVP, ex-Samsung Electronics researcher — MVP/Samsung background corroborated independently by 티타임즈TV and 헬로디디, 2026-07-29; the technique itself traced to his own primary-source blog, `tech.jehyunlee.dev`, lecture materials dated 2024-06-19 and 2024-09-25) — not a GitHub repo, cited for a specific under-specification failure mode distinct from Utley's context checks: a bare "요약해줘"("summarize this") skews the model toward the source document's abstract/intro — background framing — at the expense of the actual finding, in his words "논문의 시작 부분인 초록과 서론에 있는 연구 배경에 치중해서 이 논문에서 말하고자 하는 바가 사라지기 때문" ("[the model] leans on the background framing in the abstract and introduction, so what the paper is actually arguing gets lost"). His fix is a targeted question over a vague one — his own example, "요약해줘" → "방법론이 뭐야?" ("summarize this" → "what's the methodology?") — not a generic instruction to "critique" the source.
...
- **Task involves a paper, report, or other source document** → don't default to "summarize this" (see Attribution, 이제현). A vague summary request skews the model toward the source's abstract/intro framing and loses its actual finding; ask a targeted question instead ("what's the methodology?", "what's the actual result and how confident is it?", "where does this contradict prior work?") to pull out the part a plain summary would flatten.
