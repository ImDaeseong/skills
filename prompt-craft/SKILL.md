...
- **이제현 (Lee Je-hyun)** (Microsoft MVP, ex-Samsung Electronics researcher, now builds AI research workflows — verified via 티타임즈TV/헬로디디 interviews, 2026-07-29) — not a GitHub repo, cited for one task-framing principle distinct from Utley's context checks: treat the model as a reasoning engine, not a search/summarization engine — a bare "summarize this" produces a passive, generic pass, while handing the model the source text and instructing it to find logical flaws, gaps, or counterarguments produces a sharper, checkable result for the same input.
...
- **Task involves a paper, report, or other source document** → don't default to "summarize this" (see Attribution, 이제현). A bare summary request gets a passive, generic pass over the text; asking the model to critique the source's logic, flag gaps/counterarguments, or extract a specific decision-relevant answer gets a sharper, checkable result from the same input.
