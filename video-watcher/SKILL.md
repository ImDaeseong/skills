...
- **bradautomates/claude-video** (`watch` sub-skill) — [github.com/bradautomates/claude-video](https://github.com/bradautomates/claude-video) (16,975 stars, checked 2026-09-09 via GitHub API), **MIT license** (confirmed via the GitHub API license endpoint). Downloads with `yt-dlp`, extracts frames with `ffmpeg` (scene-aware or fast-keyframe), and transcribes via native captions or a Whisper API fallback (Groq preferred, OpenAI fallback).
...
- **Windows execution notes (verified 2026-09-09 by actually running the `watch` skill, not just reading its source):**
  - If `setup.py --json` reports `yt-dlp` as a missing binary right after a successful `pip install --user yt-dlp`, the pip user Scripts directory (typically `%APPDATA%\Python\Python3XX\Scripts`) is not on `PATH` — add it to the shell's `PATH` before re-running rather than reinstalling.
  - Set `PYTHONIOENCODING=utf-8` and `PYTHONUTF8=1` before invoking any `claude-video` script. Without them, a non-UTF-8 Windows console codepage (e.g. Korean cp949) crashes `setup.py`/`watch.py` with `UnicodeEncodeError` on their own em-dash output and mangles non-ASCII report content (a Korean video title prints as garbled bytes) — even though the underlying work (transcript, `.env` write) already completed, so the crash reads as a false failure.
  - If `--detail balanced`/`token-burner` (frame extraction) fails with `HTTP 403 Forbidden` on a YouTube source while `--detail transcript` succeeds on the same URL, yt-dlp needs a JS runtime for YouTube's signature extraction — install one (e.g. `deno`) per [yt-dlp's own guidance](https://github.com/yt-dlp/yt-dlp/wiki/EJS) before retrying frame-based detail levels.
