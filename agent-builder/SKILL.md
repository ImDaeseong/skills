...
- **Helena Liu, "How To Build AI Agents" (n8n course)** — (YouTube, 2025-02-24) — not a GitHub repo, cited as a framing source. n8n itself is proprietary/no-code, not adopted. Source of Step 1.0 — check whether the task is a fixed-sequence workflow before designing it as an agent at all; matches Anthropic's own "Building Effective Agents" workflow-vs-agent distinction, and was a genuine gap since Step 1 previously jumped straight into the five-field design without this gate.
- **Helena Liu, MCP explainer (Zapier MCP + Cursor)** — (YouTube, 2025-04-19) — not a GitHub repo. Zapier MCP itself is proprietary, not adopted. Source of a Step 1 item 3 refinement — the no-code/MCP integration-layer choice isn't strictly either/or, since some no-code platforms can expose an existing automation directly as an MCP server rather than requiring a hand-built one.
- **Samin Yasar, trading-agent tutorial** — (YouTube, 2026-06-29) — not a GitHub repo. QuiverQuant/Co-Invest/FireCrawl (the tools demoed) are proprietary or already integrated elsewhere (Firecrawl); no trading-specific tooling is adopted here. Source of a Step 1 item 5 addition, generalized beyond its finance framing: a worker whose action is irreversible (a trade, a payment, a sent message) should require its triggering signal confirmed by more than one independent source before acting, not a single feed — the video's own final maturity level was "verify across all sources."
- **Greg Isenberg, "agent-building playbook" episode** — (YouTube podcast, 2026-07-02) — not a GitHub repo, cited as a technique source. Source of a Step 1 addition: observe 10-20 real cases of a human doing the target workflow (screen-recorded, not just described) before writing the five-field definition — the exceptions and judgment calls that never make it into a verbal task summary are exactly what the fields need. Also the source of the workflow-selection traits (high frequency, clear completion point, existing software, learnable exceptions) used as a weaker-candidate flag, not a hard gate.
- **Jiaona Zhang (JZ), Laurel CPO — "How to Build a Company OS with Claude Code"** — (The Growth Podcast w/ Aakash Gupta, 2026-06-25) — not a GitHub repo; a named practitioner's real, screen-shared production system rather than a hypothetical or a proprietary-tool demo. Source of Step 5 item 0: build a task ontology (human-judgment-heavy vs. automatable) before choosing a control plane; deliver automation through the interface people already use (Slack/email) rather than a separate agent tool, since a new interface is adoption friction independent of automation quality; and assign one dedicated owner for AI-adoption rollout rather than diffusing the responsibility, proving value in one function before other teams adopt the pattern.
