...
Do not proceed to Step 2 until all five are answered. If the user can't answer one, AskUserQuestion — do not invent a plausible-sounding default. Prefer multiple-choice questions over open-ended ones here: a concrete set of options is faster for the user to react to than a blank prompt, and narrows the field to something checkable in Step 2-3 rather than another vague answer.
