...
Treat trading returns, live dashboards, and claims that an agent learns from every trade as self-reported until independently auditable. Distinguish memory or prompt updates from model training and from statistically validated strategy improvement. Require a simple baseline, time-ordered walk-forward and untouched out-of-sample evaluation, fees, spread, slippage, failed fills, turnover, drawdown, risk-adjusted return, and all attempted strategy variants before claiming improvement.
